...
module.exports = class extends Controller {
  ...
  async create(req, res) {
    try {
      ...
      if (_check == false) {
        const _data = {};
        const { email, password, status, role_id, avatar } = req.body;
  ...
  async uploadFileLocal(imageData, imagePath) {
    try {
      if (imageData && imagePath) {
        ...
        const _dir = "D:\\Legal-team\\sourceapi\\public\\upload\\avatars" || path.resolve(process.env.DIR_UPLOAD, "avatars");
  ...
  async login(req, res) {
    try {
      ...
      if (_check == false && req.account) {
        ...
        // console.log(bcrypt.hashSync("1234567", bcrypt.genSaltSync(12)));
