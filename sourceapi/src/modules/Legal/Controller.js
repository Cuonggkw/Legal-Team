  ...
  async create (req, res) {
    try {
      ...
      if(_check === false){
        ...
        if (_data?.icon) {
          try {
            const _parse = extractBase64(_data.icon.file); // `image` là key trong form upload
  
            if (!_parse) {
              return this.response(res, 400, "File ảnh không tồn tại.");
            }
            const _name = `/${nanoid()}.${_parse.ext}`;
            const filePath = path.join("./public/upload/legal_team", _name);
  
            await this.uploadFileLocal(_data.icon, filePath);
            _data.icon = filePath;
          } catch (e) {
            console.log(e);
  ...
  async uploadFileLocal(imageData, igePath) {
    try {
      if (imageData && igePath) {
        ...
        const _dir = "D:\\Legal-team\\sourceapi\\public\\upload\\legal_team" || path.resolve(process.env.DIR_UPLOAD, "legal_team");
